...
class consultas{
    ...
    public static function cargar_tiempos(){
        $contenido = '';
        include 'conexion.php';
        $consu = $mysqli->query("SELECT actividades.descripcion, fecha, hora FROM tiempos INNER JOIN actividades ON tiempos.id_actividad = actividades.id_actividad ORDER BY id_tiempo ASC");
        if($consu->num_rows > 0){
            while($r_consu = $consu->fetch_assoc()){
                $contenido .= '
                <tr>
